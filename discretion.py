...
class Discretion(object):
  def __init__(self, args):
    ...
    if "decision" not in conn.list_database_names():
      conn["decision"].create_collection("decision")
    self._decision_db = conn['decision']['decision']
  ...
  def update_state(self):
    ...
    if row:
      if row['decision'].lower() != decision.lower() and decision.lower() != "notdecided":
        self._decision.update_one(
            {"_id": row["_id"]},
            {"$set": {"decision": decision}},
            upsert=False)
        self._email_id_queue.append(row['email'])
        self._email_body_queue.append(
          f"Hey ${row['email']}, \n" \
          f"Your HR Request just got updated from ${row['decision']} to ${decision}." \
          "This is an autogenerated email, Please Don't Contact on this email")
    return decision
